...
企业现在关心的一个实际问题有关 Active Directory 环境内的资源访问问题。 特别麻烦的问题是：

- 漏洞。
- 未经授权的特权提升。
- [传递哈希](https://technet.microsoft.com/dn785092.aspx)。
- 传递票证。
- 鱼叉式网络钓鱼。
- Kerberos 泄漏。
- 其他攻击。
...
PAM 使攻击者更难入侵网络和获取特权帐户访问。 PAM 添加了对特权组的保护，以便控制对一系列加入域的计算机和这些计算机上的应用程序的访问权限。 它还改进了监视和可见性，并支持更精确地进行控制。 这样，组织可以知道谁是特权管理员，以及特权管理员在执行的操作。 PAM 可使组织更深入地了解如何在该环境中使用管理帐户。

## <a name="setting-up-pam"></a>设置 PAM

PAM 基于实时管理准则构建，该管理与[充分管理 (JEA)](http://channel9.msdn.com/Events/TechEd/NorthAmerica/2014/DCIM-B362) 相关。 JEA 是 Windows PowerShell 工具包，可定义一组用于执行特权活动的命令。 作为终结点，它可便于管理员获取运行命令的授权。 在 JEA 中，管理员决定具有特定特权的用户可以执行特定任务。 每当符合条件的用户需要执行该任务时，启用他们相应的权限。 权限在指定时间段后过期，以防恶意用户盗用访问权限。
...
获取有关 [Privileged Access Management cmdlet](https://docs.microsoft.com/powershell/identitymanager/mimpam/vlatest/mimpam) 的详细信息。
...
- [缓解传递哈希 (PtH) 攻击和其他凭据盗用 - 版本 1 和 2](https://www.microsoft.com/download/details.aspx?id=36036)
