...
## 初始化测试环境
可以使用 AD 数据测试配置之前，需要初始化该配置。 要实现此过程需完成四个步骤：
...
在初始化 MIM 连接器的过程中，配置的同步规则已经进入 metaverse。

调整由此连接器提供的属性的属性流优先顺序，以确保已在 AD 中的属性可以流入 metaverse 并且稍后还可以流入 MIM 服务数据库。

### 初始化 ADMA

若要初始化 Active Directory 连接器，需要在该连接器上运行完全导入和完全同步。 将现有的对象从 AD 带入连接器空间需要执行完全导入操作。 由于将新的同步规则从 MIM 连接器空间投影到 metaverse 更改了同步规则，因此需要执行完全同步。 你将
...
若要使用对象填充 MIM 服务数据库，需要在 MIMMA 连接器上运行同步周期。 周期包括以下运行配置文件运行：
