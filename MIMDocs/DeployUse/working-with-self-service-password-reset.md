...
title: 使用自助服务密码重置 | Microsoft 标识管理器
...
-   MIM 2016 服务&amp;门户（包括 SSPR 注册门户和 SSPR 重置门户）在服务器（可以通过同步并置）上安装并运行
...
-   集成 MIM 2016 外接程序和扩展（包括 SSPR Windows 登录）的客户端部署在服务器上或单独的客户端计算机上。
