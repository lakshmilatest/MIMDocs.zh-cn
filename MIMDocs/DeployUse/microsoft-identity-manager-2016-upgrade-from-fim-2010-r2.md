...
# 从 Forefront 标识管理器 2010 R2 升级到 Microsoft 标识管理器 2016
本部分介绍将现有测试 FIM 2010 R2 系统升级到 MIM 2016。 用于升级的安装程序也可用于全新部署。

本部分假定你在测试环境中已部署现有 FIM 2010 R2 解决方案。 你的服务器运行在 Windows Server 2012、Windows Server 2012 R2 或 Windows Server 2008 R2 上（这些操作系统是适用于 FIM 2010 R2 服务器的常见操作系统），并且为 FIM 2010 R2 配置了所有本地和环境先决条件（SQL Server、Exchange Server、SharePoint Services 等）

1.  MIM 同步服务 (Sync) 将第一次安装并在域已加入 AD 域的服务器上运行，并且替换了同步的 FIM 2010 R2 实例。

2.  然后，将安装 MIM 服务和门户（可选择包括 SSPR 注册门户和 SSPR 服务门户，并排除特权访问管理功能集）。

3.  然后，可以将集成 MIM 外接程序和扩展（包括 SSPR Windows 登录）的客户端部署到单独的客户端计算机。
