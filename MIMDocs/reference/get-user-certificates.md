...
---
# <a name="get-user-certificates"></a>获取用户证书
获取与指定用户相关联的证书列表（筛选掉已停用的证书）。
...
如果成功，将返回 JSON 序列化 [Microsoft.Clm.Shared.Certificates.X509ClmCertificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.x509clmcertificate(v=vs.100).aspx) 对象的列表，所含属性如下：
...
颁发者 | 颁发者。
NotAfter | 证书失效的日期和时间
NotBefore | 证书变为有效的日期和时间
请求er名称 | 请求证书的帐户。
SerialNumber | 证书的序列号。
状态 | 证书的状态。
TemplateCommon名称 | 证书的证书模板公用名。
